...
import { SidebarProvider, SignInPage, useSybilionAuth } from '@sybilion/uilib';
...
export function App() {
  ...
  const { isAuthenticated, isLoading } = useSybilionAuth();

  const signInRedirectPath = useMemo(
    () =>
      isAuthenticated ? getSignInRedirectPath() : WORKSPACE_PATHS.dashboard,
    [isAuthenticated],
  );
    ...
    <Routes>
      {isAuthenticated ? (
        <>
          ...
          />
          <Route
            path={WORKSPACE_PATHS.dashboard}
  ...
  const showWorkspaceShell =
    isAuthenticated && !isAuthRoute(location.pathname);
